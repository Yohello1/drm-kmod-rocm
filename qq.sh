...
for patch_path in $(ls "$PATCH_DIR"/*.patch | sort -V 2>/dev/null); do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "⚠️  Strict Git application failed. Aborting Git transaction..."
        git am --abort > /dev/null 2>&1
        
        echo "🔄 FALLBACK: Attempting lenient application via 'patch -p1'..."
        # Method 2: Fall back to standard patch tool
        patch -p1 --forward --batch < "$patch_path" > /dev/null 2>&1
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "⚠️  Standard fallback paths failed. Attempting deep file lookup..."
            ...
            if [ -n "$local_path" ]; then
                ...
                patch -p0 --forward --batch "$local_path" < "$patch_path" > /dev/null 2>&1
                LOCAL_PATCH_STATUS=$?
            else
